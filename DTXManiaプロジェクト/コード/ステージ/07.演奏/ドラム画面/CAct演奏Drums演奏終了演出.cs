...
namespace DTXMania
{
    internal class CAct演奏Drums演奏終了演出 : CActivity
    {
        ...
        public override void OnManagedリソースの作成()
        {
            ...
            this.soundFullcombo = CDTXMania.Sound管理.tサウンドを生成する(CSkin.Path(@"Sounds\Full combo.ogg"), ESoundGroup.SoundEffect);
        ...
        public override int On進行描画()
        {
            ...
            if (this.ct進行メイン != null && (CDTXMania.stage演奏ドラム画面.eフェーズID == CStage.Eフェーズ.演奏_演奏終了演出 || CDTXMania.stage演奏ドラム画面.eフェーズID == CStage.Eフェーズ.演奏_STAGE_CLEAR_フェードアウト))
            {
                ...
                {
                    if (CDTXMania.stage演奏ドラム画面.actGauge.db現在のゲージ値[i] >= 80 & CDTXMania.stage演奏ドラム画面.nヒット数_Auto含まない.Drums.Miss != 0)
                    {
