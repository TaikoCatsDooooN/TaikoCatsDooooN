...
//		}
    ...
    //            this.tx魂花火 = CDTXMania.tテクスチャの生成( CSkin.Path( @"Graphics\7_explosion_soul.png" ) );
                ...
                this.ct進行黄色 = new CCounter(0, 27, 15, CDTXMania.Timer);
                ...
                */
                ...
                if( CDTXMania.Tx.Gauge[0] != null )
                {
                    ...
                    if (this.db現在のゲージ値[0] >= 80.0)
                    {
                        ...
                        if (ct進行黄色.n現在の値 <= 14)
                        {
                            CDTXMania.Tx.Gauge[2].n透明度 = 255 - ((this.ct進行黄色.n現在の値 - 1) * (255 / 6));
                            CDTXMania.Tx.Gauge[2].vc拡大縮小倍率.Y = 1;
                            CDTXMania.Tx.Gauge[2].vc拡大縮小倍率.X = 1;
                            CDTXMania.Tx.Gauge[2].t2D描画(CDTXMania.app.Device, 492, 144, new Rectangle(0, 0, 547, 44));
                        }
                        else

                        {
                            CDTXMania.Tx.Gauge[2].n透明度 = 255 - ((this.ct進行黄色.n現在の値 - 3) * (255 / 6));
                            CDTXMania.Tx.Gauge[2].t2D描画(CDTXMania.app.Device, 492, 144, new Rectangle(0, 0, 547, 44));
                        }
                    }
